...
import { Teacher, updateTeacher, PresenceRecord } from "./teacherData";

export default function TeacherPresenceHistory({
  teacher,
  onClose,
  onSaved,
}: {
  ...
  const history = teacher.historiquePresence || [];
  ...
  const handleAdd = (e: React.FormEvent) => {
    ...
    updateTeacher(teacher.id, { historiquePresence: newHistory });
    setDate("");
    setPresent(true);
    setRefresh((r) => r + 1);
    onSaved();
  ...
  React.useEffect(() => {
    setLocalHistory(teacher.historiquePresence || []);
  }, [refresh, teacher.historiquePresence]);
